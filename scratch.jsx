...
const resolveGameKey = (event) => {
  try {
    ...
    if (pendingDeadMark) {
      const composed = composeDeadKeyMark(pendingDeadMark, key);
      pendingDeadMark = "";
      return composed || key;
    }
...
export const TypingSpaceGame = ({
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [activeMission, gameStatus, nextWordIndex, view, laneMap, missionSeed]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (wordCompleted) {
        setSpeedBoost((prev) => Math.min(prev * 1.03, 2.5));
      }
    };

    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [gameStatus, targetWord, view, combo]);
  ...
  const startMission = (mission) => {
    ...
    setFallingWords([
      createFallingWord(activeMissionInstance, 0, newLaneMap, newSeed, 1),
    ]);
    setNextWordIndex(1);
    setLives(maxLives);
    setHits(0);
  ...
  const renderSoloHome = () => (
    <div className="typingSpacePanel">
      <div className="typingSpaceMissionGrid">
        {TYPING_SPACE_MISSIONS.map((mission) => (
          <button
            type="button"
            key={mission.id}
        ...
        {gameStatus !== "running" && (
          <>
            ...
            <div className={`typingSpaceResult ${gameStatus}`}>
              ...
              <button type="button" onClick={() => startMission(activeMission)}>
  ...
  const renderScores = () => (
    <div className="typingSpaceScores">
      ...
      {loadingScores ? (
        ...
        <div className="typingSpaceEmptyScore">
            ...
            <table className="w-full text-left">
              <thead className="text-xs uppercase text-gray-400 bg-gray-50 dark:bg-[#18181b] border-b border-gray-200 dark:border-gray-800 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-6 py-3 hidden sm:table-cell">Missões</th>
                  <th className="px-6 py-3">PPM</th>
                  <th className="px-6 py-3">Precisão</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800/60">
                {scores[rankingTab].map((item, index) => (
                  ...
                  >
                    <td className="px-6 py-4 text-center font-semibold text-gray-500 dark:text-gray-500">
                      {index + 1}º
                    </td>
                    ...
                    <td className="px-6 py-4 font-mono font-medium text-[#ffcc02] dark:text-[#ffcc02]">
