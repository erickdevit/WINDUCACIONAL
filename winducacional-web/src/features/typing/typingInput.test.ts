// Porta de tests/typingInput.test.js (legado).
...
describe("Entrada de digitação com acentos", () => {
  ...
  it("deve recompor pelo caractere esperado quando o game recebe tecla morta genérica", () => {
    expect(composeDeadKeyMark("`", "a", "ã", { allowExpectedFallback: true })).toBe("ã")
  })

  it("deve identificar tecla morta mesmo quando o input ainda não mudou", () => {
    expect(resolveDeadKeyMarkFromEvent({ key: "Dead" })).toBe(UNKNOWN_DEAD_KEY_MARK)
    expect(resolveDeadKeyMarkFromEvent({ key: "Dead", code: "Backquote", shiftKey: true })).toBe("~")
  ...
  it("deve mapear tecla morta do circunflexo pelo codigo Digit6 como desconhecida", () => {
    expect(resolveDeadKeyMarkFromEvent({ key: "Dead", code: "Digit6", shiftKey: true })).toBe(
      UNKNOWN_DEAD_KEY_MARK,
    )
    expect(resolveDeadKeyMarkFromEvent({ key: "Dead", code: "Digit6", shiftKey: false })).toBe(
      UNKNOWN_DEAD_KEY_MARK,
    )
    expect(resolveDeadKeyMarkFromEvent({ key: "Dead", code: "BracketRight", shiftKey: true })).toBe("^")
  })
  ...
  it("deve ignorar acento isolado sem avançar nem contar caractere", () => {
    ...
    expect(result).toEqual({ value: "an", pendingMark: "~", ignored: true })
  })
