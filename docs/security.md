...
- O app Arcade valida todas as ações, regras de movimentação (Damas) e jogadas de cartas (Uno) no backend com validação autoritativa do servidor. O estado das salas é transmitido via SSE autenticado apenas para membros autorizados da turma. No Uno, as cartas nas mãos dos oponentes são sanitizadas e redigidas pelo servidor antes de qualquer envio de rede, transmitindo aos clientes apenas a contagem numérica de cartas para impedir qualquer tentativa de trapaça ou inspeção indevida.
