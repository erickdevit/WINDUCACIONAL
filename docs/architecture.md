...
As configurações de aprovação dos apps de digitação ficam em `typing_settings`, separadas por `student_type` (`normal` e `kids`). Professores podem criar substituições em `typing_difficulty_overrides` para uma turma ou aluno, nos modos `lesson` e `game`; a resolução efetiva é determinística e segue aluno > turma > tipo base. O painel compartilhado dos apps Kids e Normal permite selecionar a turma e o aluno, editar ambos os modos e remover a substituição para voltar à herança. Nas lições, a quantidade de erros permitidos aceita valores de 3 a 100; as vidas dos games permanecem limitadas de 3 a 10. Sessões de alunos mantêm SSE autenticado em `/api/typing/settings/events` e `/api/typing/game/settings/events`, recebendo somente a configuração efetiva do próprio usuário em tempo real, inclusive durante uma lição ou game em andamento. Rankings globais, rankings por turma e reset de pontuação também são filtrados pelo tipo da turma, impedindo mistura entre turmas Kids e Normal. O `localStorage` pode manter apenas cache de inicialização visual, mas a fonte de verdade é o PostgreSQL.

O app de Digitação Normal também contém uma área de games separada das lições tradicionais. O modo solo inicial, Defesa Orbital, usa missões próprias de palavras, nave e tiros por letra. Sua configuração base fica em `typing_game_settings`, também por `student_type`, com PPM mínimo, precisão mínima, vidas do game, velocidade percentual de descida das palavras e aceleração percentual por palavra destruída; as substituições direcionadas usam a mesma precedência e não contaminam as lições. O app Kids compartilha a mesma resolução para seu game e PVP, incluindo vidas e metas visuais. Alunos recebem mudanças em tempo real por SSE em `/api/typing/game/settings/events`, e os rankings oficiais do game usam `typing_game_scores` e a view `typing_game_ranking`, com listagens global e por turma e reset próprio por código de turma. O histórico em `localStorage` pode existir apenas como fallback visual quando a API não estiver disponível. O modo PVP usa endpoints autenticados em `/api/typing-pvp/*` para lobby, convite, pareamento aleatório, sincronização SSE, finalização e histórico persistido em `typing_pvp_matches`; os placares são listados por escopo global ou turma, filtrando a turma do aluno ou a turma selecionada pelo professor. A finalização atual identifica os participantes da sala no servidor e persiste a pontuação enviada pelo cliente; a validação autoritativa de cada palavra digitada no servidor permanece como etapa futura de endurecimento.
...
O app Desenho da Turma usa `drawing_activities` e `drawing_strokes` para atividades vinculadas a uma turma. O professor cria uma atividade com tema, orientações, cor de fundo e modo `individual` ou `chaos`; o primeiro mantém um desenho por aluno, oferece mosaico completo da turma e permite a escolha persistida de vencedor, enquanto o segundo mantém um único quadro coletivo. O histórico combina turma, tema, modo, participação, desenho vencedor e data de encerramento. Os traços são coordenadas normalizadas, validadas e persistidas pelo backend, e mudanças chegam por SSE autenticado em `/api/drawing/activities/:id/events`. No modo caos, novos traços são acrescentados atomicamente no PostgreSQL para impedir que desenhos simultâneos se sobrescrevam.

Conexões SSE específicas de aplicativos permanecem abertas somente enquanto a respectiva janela está visível. Ao minimizar ou fechar Digitação, Digitação Kids ou Desenho da Turma, o frontend encerra as assinaturas e volta a carregar o estado persistido ao reabrir o app. Isso preserva conexões HTTP para operações administrativas, como listar e criar turmas, sem perder sincronização durante o uso efetivo da atividade.
