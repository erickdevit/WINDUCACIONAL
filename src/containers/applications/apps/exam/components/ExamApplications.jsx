...
const getSubmissionStatus = (item) => {
  ...
  if (item.removalStatus === "retained" && item.assignmentStatus !== "created")
    return "retained";
...
export const ExamApplications = () => {
  ...
  useEffect(() => {
    if (
      ...
      !visibleApplications.find((a) => a.id === selectedApplicationId)
    ) {
        ...
        <div className="exam-filters">
          ...
          <div className="flex gap-2 items-center">
            {cancelledCount > 0 && (
              <button
                ...
                className={`btn-secondary compact ${
                  !showCancelled ? "active" : ""
                }`}
                onClick={() => setShowCancelled(!showCancelled)}
                title={
                  showCancelled ? "Ocultar removidas" : "Mostrar removidas"
                }
              >
                <Icon
                  fafa={showCancelled ? "faEyeSlash" : "faEye"}
                  width={12}
                />
                {showCancelled ? "Ocultar removidas" : "Mostrar removidas"} (
                {cancelledCount})
            ...
            <button
              type="button"
              className="btn-secondary compact"
              onClick={() => loadData(selectedTurmaId)}
            >
            ...
            {visibleApplications.map((application) => (
              <button
                ...
                className={`application-row ${
                  selectedApplicationId === application.id ? "active" : ""
                }`}
              ...
              >
                ...
                <small>
                  {application.totalCreated} criada
                  {application.totalCreated === 1 ? "" : "s"} ·{" "}
                  {application.totalExisting} existente
                  {application.totalExisting === 1 ? "" : "s"}
                </small>
        ...
        <div className="application-detail exam-panel">
          <div className="application-detail-head">
            <div>
              ...
              <span>
                {selectedApplication?.appliedBy?.displayName || "Professor"} ·{" "}
                {selectedApplication?.modeLabel || "-"}
              </span>
              {selectedApplication?.cancelledAt && (
                <span>
                  Removida por{" "}
                  {selectedApplication.cancelledBy?.displayName || "professor"}{" "}
                  em {formatDateTime(selectedApplication.cancelledAt)}
                </span>
              )}
            </div>
              ...
              {visibleItems.map((item) => {
                ...
                return (
                  <article key={item.id} className="exam-mobile-card">
                    <h4>
                      {item.displayName || "Aluno indisponível"}{" "}
                    ...
                    <div className="card-info-row mt-2">
                      <div className="flex flex-col gap-1">
                        ...
                        <span
                          className={`trace-pill ${submissionStatus}`}
                          style={{
                            width: "fit-content",
                            padding: "2px 6px",
                            fontSize: "10px",
                    ...
                    <div className="card-info-row">
                      <div className="flex flex-col gap-1">
                        ...
                        <span
                          className={`trace-pill ${item.assignmentStatus}`}
                          style={{
                            width: "fit-content",
                            padding: "2px 6px",
                            fontSize: "10px",
                        ...
                        >
                          {assignmentLabels[item.assignmentStatus] ||
                            item.assignmentStatus}
                        </span>
                      </div>
                      <div className="flex flex-col gap-1 text-right">
                        <span className="card-label">Conclusão</span>
                        <span className="card-value font-normal text-xs text-muted">
