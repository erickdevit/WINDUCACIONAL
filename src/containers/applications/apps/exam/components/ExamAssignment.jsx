...
export const ExamAssignment = () => {
  ...
  const handleApply = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.assignExamBatch({ assignments, mode: distributeMode });
          ...
          <div className="exam-panel p-5">
            ...
            <div className="mb-4">
              ...
              >
                ...
                {turmas.map(t => <option key={t.id} value={t.id}>{t.nome} ({t.code})</option>)}
              </select>
            </div>
                ...
                <table className="exam-student-table">
                  <thead>
                    <tr>
                      ...
                      <th className="p-3 font-bold text-gray-500 uppercase text-[10px]">Aluno</th>
                      <th className="p-3 font-bold text-gray-500 uppercase text-[10px]">Usuário</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {students.map(s => (
                      <tr key={s.id} className="hover:bg-blue-50 transition cursor-pointer" onClick={() => toggleStudent(s.id)}>
                        <td className="p-3">
                          <input 
                            type="checkbox" 
                        ...
                        <td className="p-3 font-bold text-gray-700">{s.displayName}</td>
                        <td className="p-3 text-gray-400">@{s.username}</td>
                      </tr>
                    ))}
                    {students.length === 0 && (
                      <tr>
                        <td colSpan={3} className="p-10 text-center text-gray-400">Nenhum aluno nesta turma.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        <div className="space-y-4">
          <div className="exam-panel p-5">
              ...
              >
                ...
                {exams.map(e => (
                  <option key={e.id} value={e.id}>
                    {e.title}{e.timeLimit > 0 ? ` (${e.timeLimit} min)` : ""}
                  </option>
                ))}
              </select>
              <button type="button" className="btn-secondary" onClick={addSelectedExam}>
                Adicionar
              </button>
            </div>
            ...
            <div className="selected-exam-list win11Scroll">
                ...
                return (
                  <div key={id} className="selected-exam-row">
                    <span>
                      ...
                      <small>{exam.timeLimit > 0 ? `${exam.timeLimit} min` : "Sem limite"}</small>
                    </span>
                    <button type="button" onClick={() => toggleExam(id)} title="Remover prova">
              ...
              {exams.length === 0 ? (
                <div className="exam-empty compact">Nenhuma prova publicada.</div>
              ) : selectedExamIds.length === 0 && (
                <div className="exam-empty compact">Nenhuma prova selecionada.</div>
              )}
            </div>
          </div>
          ...
          <div className="exam-panel p-5">
            ...
            <button 
              disabled={loading || selectedStudentIds.length === 0 || selectedExamIds.length === 0}
              className="w-full btn-primary"
              onClick={handleApply}
            >
              {loading ? 'Processando...' : `Aplicar para ${selectedStudentIds.length}`}
            </button>
          </div>
        </div>
      </div>
