      ...
      <div className="mb-8">
        <div className="flex justify-between items-end mb-2">
          <div>
            <span className="text-sm font-medium text-blue-600 uppercase tracking-wider">
              Questão {currentIndex + 1} de {mcqQuestions.length}
            </span>
            <span className="block text-xs text-gray-400">
              {Math.round(((currentIndex + 1) / mcqQuestions.length) * 100)}%
              concluído
            </span>
          </div>
          {q.timeLimit > 0 && (
        ...
        <div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${((currentIndex + 1) / mcqQuestions.length) * 100}%`,
            }}
          ></div>
        </div>
      </div>
          ...
          {q.options.map((opt, i) => {
            ...
            return (
              ...
              >
                ...
                />
                <span
                  className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center mr-4 ${
                    isSelected
