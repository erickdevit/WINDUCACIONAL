...
- O app Montagem de PC usa o catálogo e as regras compartilhadas em `server/domain/pcBuilder*.mjs`. Toda tentativa de ligar o computador deve ser validada novamente pelo backend, e o histórico em `pc_builds` deve permanecer isolado pelo usuário autenticado. Alterações de compatibilidade precisam manter frontend, API e testes de domínio sincronizados. As ilustrações vetoriais ficam centralizadas em `ComponentArtwork.jsx`; as vistas 2D e 3D devem representar a mesma seleção e continuar acessíveis por controles nativos.
- Os apps de Digitação Kids e Normal mantêm dificuldade base por tipo em `typing_settings`/`typing_game_settings` e substituições direcionadas em `typing_difficulty_overrides`. A resolução é sempre aluno > turma > tipo, validada no backend, para lições e games; o painel compartilhado deve manter os dois modos sincronizados por SSE. Games dentro do app de Digitação Normal devem ficar separados das lições tradicionais. O modo PVP só deve ser liberado como partida real depois de existir backend com convite, sincronização em tempo real, validação de mesma turma, cálculo de vencedor no servidor e ranking persistido separado.
- O app Desenho da Turma mantém atividades por turma em `drawing_activities` e traços vetoriais em `drawing_strokes`. Os modos individual e caos usam SSE autenticado; o servidor valida limites dos pontos, atividade ativa, papel e turma antes de persistir ou transmitir dados. Alunos recebem somente o próprio desenho no modo individual; professores recebem o mosaico completo. No modo caos, traços devem ser anexados atomicamente para evitar sobrescrita entre alunos. Somente professor pode escolher como vencedor um aluno da turma que tenha desenho persistido, e o histórico deve preservar o desenho vencedor.
- O app Arcade oferece jogos multiplayer em sala com colegas de turma, iniciando com o jogo de Damas (duplas - 2 jogadores) e o jogo de Uno (mínimo de 3 até a turma toda). O estado das salas é gerenciado em `arcade_rooms` e `arcade_room_players` com regras puras de domínio no backend (`arcadeCheckers.cjs` e `arcadeUno.cjs`), streaming SSE autenticado, sanitização das cartas dos oponentes contra trapaças no Uno e rankings persistidos por turma e geral da escola em `arcade_rankings`.
- Assinaturas SSE específicas de apps devem existir somente enquanto a janela correspondente estiver visível. Ao ocultar um app, encerre sua conexão para não esgotar o limite de conexões do navegador e bloquear requisições administrativas.
